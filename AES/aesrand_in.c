...
inline void AES_Encryption() {
	...
	printf("input XOR key: %08x %08x %08x %08x\n", in0, in1, in2, in3);
...
void generateRandomInput(char* random_source, int i, int bit_value) {
	// Generate a random 32-digit hex string
	for (int j = 0; j < 4; ++j) {
		char buffer[CHUNK_SIZE + 1] = {0};
		strncpy(buffer, random_source + i + j * CHUNK_SIZE, CHUNK_SIZE);
		P[j] = (unsigned int)strtoul(buffer, NULL, 16);
	}
	u32 mask = 0x00FFFFFF;
	P[0] &= mask;
	P[0] |= (bit_value) << 24;
...
int main(int argc, char* argv[]) {
	...
	int bit_value = atoi(argv[2]);
	char *random_source = argv[3];
	...
	for (int i = 0; i < iteration; i++) {
		printf("Iteration %d\n", i);
		generateRandomInput(random_source, i, bit_value);
